...
// This file contains the implementation of many of the assertions used in test suites
// across gecko. It was originally imported from narwhal.js, which followed the unit
// testing spec from CommonJS. It has since been updated with class syntax.
//
// Original spec: http://wiki.commonjs.org/wiki/Unit_Testing/1.0
//
...
var EXPORTED_SYMBOLS = ["Assert", "AssertLibrary", "AssertionError"];
...
/**
 * A reporter function for assertions.
 *
 * @typedef {Function} ReporterFunction
 * @param {Error} [error] - The error to report
 * @param {any} [message] - The message to report.
 * @param {any} [stack] - The stack of the error.
 */

/**
 * The library for Assert.jsm. This module is often used statically through the
 * exported `Assert` instance. However, some test runners may prefer creating their
 * own Assert instance.
 */
class AssertLibrary {
  /**
   * The reporter for the instance.
   * @type {ReporterFunction | null};
   */
  #reporter = null;
   ...
   * ```
   *
   * @param {boolean} failed - Indicates if the assertion failed or not
   * @param {unknown} actual - The result of evaluating the assertion
   * @param {unknown} [expected] - Expected result from the test author
   * @param {string} [message] - Short explanation of the expected result
   * @param {string} [operator] - Operation qualifier used by the assertion
   *                              method (example: '==')
   */
  report(
    failed,
    ...
    if (!reporter) {
      ...
      reporter(failed ? err : null, err.message, err.stack);
    }
  };
   ...
   * message_opt);.
   *
   * @param {unknown} value -Test subject to be evaluated as truthy
   * @param {string} [message] - Short explanation of the expected result
   */
  ok(value, message) {
    if (arguments.length > 2) {
      ...
      return this.report(!value, value, true, message, "==");
    }
  };
   ...
   *
   * @param {unknown} actual - Test subject to be evaluated as equivalent to `expected`
   * @param {unknown} expected - Test reference to evaluate against `actual`
   * @param {string} [message] - Short explanation of the expected result
   */
   ...
   *
   * @param {unknown} actual - Test subject to be evaluated as NOT equivalent to `expected`
   * @param {unknown} expected - Test reference to evaluate against `actual`
   * @param {string} [message] - Short explanation of the expected result
   */
   ...
   *
   * @param {unknown} actual - Test subject to be evaluated as equivalent to `expected`, including nested properties
   * @param {unknown} expected - Test reference to evaluate against `actual`
   * @param {string} [message] - Short explanation of the expected result
   */
   ...
   *
   * @param {unknown} actual - Test subject to be evaluated as NOT equivalent to `expected`, including nested properties
   * @param {unknown} expected - Test reference to evaluate against `actual`
   * @param {string} [message] - Short explanation of the expected result
   */
   ...
   *
   * @param {unknown} actual - Test subject to be evaluated as strictly equivalent to `expected`
   * @param {unknown} expected - Test reference to evaluate against `actual`
   * @param {string} [message] - Short explanation of the expected result
   */
   ...
   *
   * @param {unknown} actual - Test subject to be evaluated as NOT strictly equivalent to `expected`
   * @param {unknown} expected - Test reference to evaluate against `actual`
   * @param {string} [message] - Short explanation of the expected result
   */
   ...
   * ```
   *
   * @param {function} block - Function block to evaluate and catch eventual thrown errors
   * @param {any} expected - This parameter can be either a RegExp or a function. The
   *        function is either the error type's constructor, or it's a method that
   *        returns a boolean that describes the test outcome.
   * @param {string} [message] - Short explanation of the expected result
   */
   ...
   *
   * @param {promise} promise - A promise that is expected to reject
   * @param {unknown} [expected] - Test reference to evaluate against the rejection result
   * @param {string} [message] - Short explanation of the expected result
   */
   ...
   * @returns {boolean}
   */
  #compareNumbers(expression, lhs, rhs, message, operator) {
   ...
   *
   * @param {number} lhs - The left-hand side value
   * @param {number} rhs - The right-hand side value
   * @param {string} [message] - Short explanation of the comparison result
   */
  greater(lhs, rhs, message) {
    this.#compareNumbers(lhs <= rhs, lhs, rhs, message, ">");
  };
   ...
   * @param {string} [message] - Short explanation of the comparison result
   */
  greaterOrEqual(lhs, rhs, message) {
    this.#compareNumbers(lhs < rhs, lhs, rhs, message, ">=");
  };
   ...
   *
   * @param {string} lhs - The string to be tested.
   * @param {string | RegExp} rhs - The regular expression that the string will be
   *        tested with. Note that if passed as a string, this will be interpreted
   *        as a regular expression.
   * @param {string} [message] - Short explanation of the comparison result
   */
  stringMatches(lhs, rhs, message) {
   ...
   *
   * @param {string} lhs - The string to be tested.
   * @param {string} rhs - The string to be found.
   * @param {string} [message] - Short explanation of the comparison result
   */
  stringContains(lhs, rhs, message) {
...
/**
 * An assertion library that can be used across different test suites. It can be
 * customized with different reporter functions.
 */
const Assert = new AssertLibrary(null);

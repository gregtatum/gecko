...
template <typename T, typename Result, typename... Args>
void SpawnOSBackgroundThread(T& aReceiver, dom::Promise& aPromise,
  ...
  // > members.
  auto holder =
      MakeRefPtr<nsMainThreadPtrHolder<T>>("nsTextRecognition::SpawnOSBackgroundThread", &aReceiver,
                                           /* strict = */ false);
...
template <typename T, typename Result, typename... Args>
nsresult AsyncPromise(T& aReceiver, RefPtr<dom::Promise>& aPromiseSlot, JSContext* aCx,
                      dom::Promise** aResultPromise, CallOS<T, Result, Args...> aTask,
                      Args... aArgs) {
  ...
  SpawnOSBackgroundThread(aReceiver, *promise, aTask, std::forward<Args>(aArgs)...);
...
NS_IMETHODIMP
nsTextRecognition::FindText(imgIContainer* aImage, JSContext* aCx,
  ...
  return AsyncPromise(*this, mCallPromise, aCx, aResultPromise, &nsTextRecognition::CallOS,
                      surface);
}
  ...
  if (@available(macOS 10.15, *)) {
    ...
    VNRecognizeTextRequest* textRecognitionRequest = [[VNRecognizeTextRequest alloc]
        initWithCompletionHandler:^(VNRequest* _Nonnull request, NSError* _Nullable error) {
          ...
          [observations enumerateObjectsUsingBlock:^(VNRecognizedTextObservation* _Nonnull obj,
    ...
    // CPU call.
    NSError* error = nil;
    VNImageRequestHandler* requestHandler = [[VNImageRequestHandler alloc] initWithCGImage:imageRef
                                                                                   options:@{}];

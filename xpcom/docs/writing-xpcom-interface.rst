...
They can also be accessed manually in JavaScript and in C++ code, through somewhat arcane invocations involving their **contract id**:
...
First decide on a name. Conventionally either :code:`nsI` (historically Netscape) or :code:`mozI` are used to prefix these interfaces as they are defined in the global namespace. The implementation of an interface can be defined in a namespace, but the :code:`ns` and :code:`moz` prefixes are often retained in the implementation, purely by convention. This can help to indicate that it's implementing the interface.

This tutorial assumes the component is located at :code:`path/to` with the name :code:`ComponentName`. The interface name will be :code:`nsIComponentName`, while the implementation will be :code:`mozilla::nsComponentName` (note the dropped :code:`I`).
...
.. code:: c++
    ...
    // `uuid`       Generate your own here: http://mozilla.pettay.fi/cgi-bin/mozuuid.pl
    //              This is also known as the CID.

    /**
     * Make sure to document your interface.
     */
    [scriptable, uuid(9a851baa-c794-4532-9042-79c7f2d717c4)]
...
Now write the header: :code:`path/to/nsComponentName.h`
...
.. code:: c++
    ...
    // Notice how the class name removes the `I`. This is a common convention.
    class nsComponentName final : public nsIComponentName {
...
Now write the definitions: :code:`path/to/nsComponentName.cpp`
...
.. code:: c++
    ...
    #include "nsComponentName.h"

    namespace mozilla {

    // Use the macro to inject all of the definitions for nsISupports.
    NS_IMPL_ISUPPORTS(nsComponentName, nsIComponentName)
    ...
    NS_IMETHODIMP
    nsComponentName::GetIsAlive(bool* aIsAlive) {
      *aIsAlive = true;
      return NS_OK;
    }
...
.. code:: python
    ...
    Classes = [
        {
            # This is the uuid that was defined in the .idl file.
            ...
            # solution for the Gecko Runtime Environment (GRE). At this point everything in
            # mozilla-central is has an @mozilla domain.
            'contract_ids': ['@mozilla.org/component-name;1'],

            # This is the name of the C++ type that implements the interface.
            'type': 'mozilla::nsComponentName',

            # The header file to pull in for the implementation of the interface.
            'headers': ['path/to/nsComponentName.h'],

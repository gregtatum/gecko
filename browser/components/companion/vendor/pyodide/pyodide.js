!(function(global, factory) {
  ...
  const IN_NODE =
    "undefined" != typeof process && "undefined" !== process.release.name;
  let baseURL;
  ...
  function recursiveDependencies(
    ...
    if (sharedLibsOnly) {
      ...
      for (let c of toLoad) {
        let name = c[0];
        Module.packages[name].shared_library &&
  ...
  async function _loadPackage(names, messageCallback, errorCallback) {
    ...
    let resolveMsg,
      packageList = [];
    for (let [pkg, uri] of toLoad) {
      (loadedPackages[pkg] = uri), packageList.push(pkg);
    }
    ...
    (Module.pyproxy_new = function(ptrobj, cache) {
      ...
      cache.refcnt++,
        Object.defineProperty(target, "$$", {
          value: { ptr: ptrobj, type: "PyProxy", borrowed: !1, cache },
        }),
  ...
  (Module.pyproxy_destroy = function(proxy, destroyed_msg) {
    ...
    (Module.callPyObjectKwargs = function(ptrobj, ...jsargs) {
      ...
      let idresult,
        idargs = Module.hiwire.new_value(jsargs),
        idkwnames = Module.hiwire.new_value(kwargs_names);
      try {
  ...
  class PyProxyClass {
    ...
    toString() {
      ...
      return (
        0 === jsref_repr && Module._pythonexc2js(),
    ...
    toJs({
      ...
      (proxies_id = create_pyproxies
        ? pyproxies
          ? Module.hiwire.new_value(pyproxies)
          : Module.hiwire.new_value([])
        : 0),
        dict_converter &&
  ...
  class PyProxyGetItemMethods {
    get(key) {
      let idresult,
        ptrobj = _getPtr(this),
        idkey = Module.hiwire.new_value(key);
      try {
  ...
  class PyProxyIteratorMethods {
    ...
    next(arg) {
      let idresult,
        ...
        idarg = Module.hiwire.new_value(arg);
      try {
      ...
      return (
        done && 0 === idresult && Module._pythonexc2js(),
        { done, value: Module.hiwire.pop_value(idresult) }
      );
    }
  }
  let PyProxyHandlers = {
      ...
      ("symbol" != typeof jskey &&
        ...
        (function(jsobj, jskey) {
          let result,
            ...
            idkey = Module.hiwire.new_value(jskey);
          try {
  ...
  class PyProxyCallableMethods {
    ...
    callKwargs(...jsargs) {
      ...
      if (
        void 0 !== kwargs.constructor &&
        "Object" !== kwargs.constructor.name
      ) {
  ...
  class PyProxyBufferMethods {
    getBuffer(type) {
      ...
      let startByteOffset = HEAPU32[0 + (buffer_struct_ptr >> 2)],
        ...
        strides = Module.hiwire.pop_value(
          HEAPU32[7 + (buffer_struct_ptr >> 2)]
        ),
  ...
  let globalsPropertyAccessWarned = !1,
    globalsPropertyAccessWarningMsg =
      "Access to pyodide.globals via pyodide.globals.key is deprecated and will be removed in version 0.18.0. Use pyodide.globals.get('key'), pyodide.globals.set('key', value), pyodide.globals.delete('key') instead.",
    NamespaceProxyHandlers = {
      has: (obj, key) => Reflect.has(obj, key) || obj.has(key),
      get(obj, key) {
  ...
  async function loadPackagesFromImports(code, messageCallback, errorCallback) {
    let imports,
      pyimports = Module.pyodide_py.find_imports(code);
    try {
  ...
  function pyimport(name) {
    return (
      console.warn(
        "Access to the Python global namespace via pyodide.pyimport is deprecated and will be removed in version 0.18.0. Use pyodide.globals.get('key') instead."
      ),
  ...
  function toPy(obj, { depth: depth = -1 } = {}) {
    ...
    try {
      if (
        ...
        0 === py_result && Module._pythonexc2js(),
        Module._JsProxy_Check(py_result))
      ) {
        return obj;
      }
  ...
  async function loadPyodide(config) {
    ...
    if (
      ...
      globalThis.__pyodide_module)
    ) {
      throw globalThis.languagePluginURL
        ? new Error(
            "Pyodide is already loading because languagePluginURL is defined."
          )
        : new Error("Pyodide is already loading.");
    }
    if (
    ...
    let pyodide = (function() {
      ...
      return (
        (namespace._module = Module), (Module.public_api = namespace), namespace
      );
    })();
    var ns;
    return (
      ...
      (Module.globals =
        ((ns = Module.globals), new Proxy(ns, NamespaceProxyHandlers))),
  ...
  (Module.fatal_error = function(e) {
    ...
    (Module.runPythonSimple = function(code) {
      let errcode,
        code_c_string = Module.stringToNewUTF8(code);
      try {
    ...
    globalThis.languagePluginUrl &&
      (console.warn(
        "languagePluginUrl is deprecated and will be removed in version 0.18.0, instead use loadPyodide({ indexURL : <some_url>})"
      ),

...
class EngagementChild extends JSWindowActorChild {
  ...
  receiveMessage(msg) {
    if (msg.name == "Engagement:Categorize") {
      ...
      return Promise.resolve(category(this.document));
    }
  }
}
...
function category(document) {
  ...
  const scores = {
    "shopping": shoppingFnodes.length === 0 ? 0 : shoppingFnodes[0].scoreFor("shopping"),
    "article": articleFnodes.length === 0 ? 0 : articleFnodes[0].scoreFor("article"),
  };

...
import { AccountsListView } from "./accounts_list_view";
 ...
 * to the folder id's.
 */
const normalizeFoldersToIds = folders => folders?.map(folder => folder.id);
...
export class MailAPI extends Emitter {
  ...
  viewAccounts(opts) {
    const handle = this._nextHandle++,
      view = new AccountsListView(this, handle, opts);
  ...
  parseMailbox(email) {
    try {
      const mailbox = addressparser.parse(email);

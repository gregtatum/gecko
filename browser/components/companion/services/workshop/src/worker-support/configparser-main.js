...
function parseAccountCommon(uid, cmd, text) {
  ...
  if (incoming) {
    ...
    if (incoming.getAttribute("type") === "activesync") {
      ...
      if (
        !ALLOWED_SOCKET_TYPES.includes(config.incoming.socketType) ||
        !ALLOWED_SOCKET_TYPES.includes(config.outgoing.socketType)
      ) {
        config = null;
        status = "unsafe";
      }
...
function parseActiveSyncAccount(uid, cmd, text, aNoRedirect) {
  ...
  var getString = function(xpath, rel) {
    return doc.evaluate(xpath, rel, nsResolver, XPathResult.STRING_TYPE, null)
      .stringValue;
  };
  ...
  var responseNode =
    getNode("/ad:Autodiscover/ms:Response", doc) ||

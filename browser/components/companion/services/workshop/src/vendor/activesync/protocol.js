...
import { Reader, Writer } from "wbxml";
import ASCP from "./codepages"
...
export var AutodiscoverError = makeError('ActiveSync.AutodiscoverError');

export var AutodiscoverDomainError = makeError('ActiveSync.AutodiscoverDomainError',
                                        AutodiscoverError);

export var HttpError = makeError('ActiveSync.HttpError', null, ['status']);
 ...
 */
export function autodiscover(aEmailAddress, aPassword, aTimeout, aCallback,
                      aNoRedirect) {
...
export function raw_autodiscover(aUrl, aEmailAddress, aPassword, aTimeout,
                          aNoRedirect, aCallback) {
 ...
 */
export function Connection(aDeviceId, aDeviceType) {
      ...
      xhr.onload = function() {
        ...
        if (isMultipart) {
          ...
          if (xhr.response.byteLength > 0) {
            response = new Reader(new Uint8Array(xhr.response), ASCP);
          }
          if (conn.onmessage) {
            conn.onmessage(

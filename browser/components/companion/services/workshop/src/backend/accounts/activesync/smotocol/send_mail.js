...
import { Writer } from "wbxml";
import cm from "activesync/codepages/ComposeMail";
...
export default async function sendMail(conn, { mimeBlob, progress }) {
  ...
  w.stag(cm.Tags.SendMail)
    ...
    .tag(cm.Tags.ClientId, Date.now().toString() + "@mozgaia")
    .tag(cm.Tags.SaveInSentItems)
    .stag(cm.Tags.Mime)
    .opaque(mimeBlob)
    .etag()
    .etag();

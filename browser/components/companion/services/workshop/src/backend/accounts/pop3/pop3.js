...
import {
  POP3_SNIPPET_SIZE_GOAL,
  POP3_INFER_ATTACHMENTS_SIZE,
} from "../../syncbase";
...
import ByteCounterTransformStream from "../../streamy/byte_counter_transform_stream";
...
function Pop3Client(options, cb) {
  ...
  if (!options.port) {
    ...
    if (!options.port) {
      throw new Error(
        "Invalid crypto option for Pop3Client: " + options.crypto
      );
    }
  }
  ...
  greetingRequest.then(
    () => {
      ...
      this._maybeUpgradeConnection(
        function(err) {
          if (err) {
            cb && cb(err);
            return;
          }
          this._thenAuthorize(function(err2) {
            if (!err2) {
              this.state = "ready";
            }
            cb && cb(err2);
  ...
  this.requestStream = new transport.Pop3RequestStream(
    this.socket,
    greetingRequest
  );
}
...
Pop3Client.prototype._maybeUpgradeConnection = function(cb) {
  if (this.options.crypto === "starttls") {
    this.state = "starttls";
    this.sendRequest("STLS", [], false).then(
      () => {
  ...
  switch (this.authMethod) {
    case "apop":
      ...
      if (!apopTimestamp) {
        ...
        this.sendRequest("APOP", [user, secret], false).then(
          () => {
            cb(); // ready!
          },
          err => {
    ...
    case "sasl":
      ...
      this.sendRequest("AUTH", ["PLAIN", secret], false).then(
        () => {
          cb(); // ready!
        },
        err => {
    ...
    case "user-pass":
    default:
      this.sendRequest("USER", [user], false).then(
        () => {
          this.sendRequest("PASS", [pass], false).then(
            () => {
              cb();
            },
            err => {
...
Pop3Client.prototype.quit = function(cb) {
  this.state = "disconnected";
  this.sendRequest("QUIT", [], false).then(
    () => {
      this.close();
      cb && cb();
    },
...
Pop3Client.prototype._loadMessageList = function() {
  ...
  return new Promise((resolve, reject) => {
    ...
    this.sendRequest("UIDL", [], true).then(
      ({ dataLines }) => {
    ...
    this.sendRequest("LIST", [], true).then(
      ({ dataLines }) => {
        var allMessages = [];
        for (var i = 0; i < dataLines.length; i++) {
          var words = dataLines[i].split(" ");
...
Pop3Client.prototype.listMessages = function(opts, cb) {
  ...
  this._loadMessageList().then(
    unfilteredMessages => {
      ...
      console.log(
        "POP3: listMessages found " +
          messages.length +
          " new, " +
          overflowMessages.length +
          " overflow, and " +
          seenCount +
          " seen messages. New UIDLs:"
      );
...
Pop3Client.prototype.downloadMessage = async function(uidl, handlers) {
  ...
  try {
    var ret = await this.parseMessageFromLineStream(
      request.dataLineStream,
      uidl,
      this.idToSize[number],
      handlers
    );
  } catch (e) {
...
Pop3Client.prototype.parseMessageFromLineStream = async function(
  lineStream,
  srvid,
  totalExpectedSize,
  handlers
  ...
  for (;;) {
    ...
    if (!done) {
        ...
        rep.file = await mimeStreams.readAttachmentStreamWithChunkFlushing(
          ...
          async function(file) {
            ...
            if (handlers.flushBodyInfo) {
              partBuilder.body = await handlers.flushBodyInfo(partBuilder.body);
            }
            if (type === "attachment") {
      ...
      // For now, if it's a body, we just concatenate everything to a string.
        ...
        rep.content = mimefuncs.charset.decode(
      ...
      // middle.
      header.bytesToDownloadForBodyDisplay = partiallyDownloaded
        ? totalExpectedSize
        : 0;
...
export { Pop3Client, setTimeoutFunctions };

 ...
 *
 * - sanitize HTML (using https://searchfox.org/mozilla-central/source/parser/html/nsIParserUtils.idl):
 *   discard illegal markup entirely, render legal but 'regulated' markup inert
     (ex: links to external content).
...
import { callOnMainThread } from "../worker-router";
...
// TODO: check if we want implement includeQuotes.
// For info, it's a matter of having blockquote content
// in the output.
// It's likely something useful with email contents
// but useless with calendar contents.
// Anyway, it's something we can easily add in parserUtils.
export function generateSnippet(htmlString, includeQuotes) {
  return callOnMainThread({
 ...
 *
 */
export function wrapTextIntoSafeHTMLString(
  text,
  wrapTag,
  transformNewlines,
  attrs
  ...
  for (let i = 0, ii = attrs?.length || 0; i < ii; i += 2) {
    attributes += ` ${attrs[i]}="${escapePlaintextIntoAttribute(
      attrs[i + 1]
    )}"`;
  }

...
const kEnableTranslationsPref = "browser.translations.enable";
...
function test() {
  ...
  registerCleanupFunction(function() {
    ...
    Services.prefs.clearUserPref(kAlwaysTranslateLangsPref);
    Services.prefs.clearUserPref(kNeverTranslateLangsPref);
  });
...
function getAlwaysTranslateLanguages() {
  let langs = Services.prefs.getCharPref(kAlwaysTranslateLangsPref);
  return langs ? langs.split(",") : [];
}

function getNeverTranslateLanguages() {
  let langs = Services.prefs.getCharPref(kNeverTranslateLangsPref);
  return langs ? langs.split(",") : [];
}
...
var gTests = [
...

  {
    ...
    run: async function testAlwaysTranslateLanguages() {
      ...
      Services.prefs.setCharPref(kAlwaysTranslateLangsPref, "fr,de");

      // Open the translations preferences dialog.
      let win = openDialog(
      ...
      // Check that the list of preferences is loaded.
      ...
      is(getAlwaysTranslateLanguages().length, 1, "One langtag in the pref");
      ...
      Services.prefs.setCharPref(kAlwaysTranslateLangsPref, "fr,en,es");
      is(
        tree.view.rowCount,
        3,
      ...
      ok(removeAll.disabled, "The 'Remove All Languages' button is disabled");
      is(
        Services.prefs.getCharPref(kAlwaysTranslateLangsPref),
        "",
        "The pref is empty"
      );
...

  {
    ...
    run: async function testNeverTranslateLanguages() {
      // Put 2 languages in the pref before opening the window to check
      // the list is displayed on load.
      Services.prefs.setCharPref(kNeverTranslateLangsPref, "fr,de");
      ...
      // Check that the list of language preferences is loaded.
      let getById = win.document.getElementById.bind(win.document);
      ...
      // Add an item and check it appears.
      Services.prefs.setCharPref(kNeverTranslateLangsPref, "fr,en,es");
      is(
        tree.view.rowCount,
        3,
      ...
      ok(removeAll.disabled, "The 'Remove All Languages' button is disabled");
      is(
        Services.prefs.getCharPref(kNeverTranslateLangsPref),
        "",
        "The pref is empty"
      );
...

  {
    ...
    run: async function testNeverTranslateSites() {
      ...
      // Check that the list of site preferences is loaded.
      let getById = win.document.getElementById.bind(win.document);
      ...
      PermissionTestUtils.remove("https://example.org", "translate");
      PermissionTestUtils.remove("https://example.com", "translate");

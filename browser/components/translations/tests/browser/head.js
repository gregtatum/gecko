 ...
 * @param {string} options.l10nId - The data-l10n-id attribute value of the element to retrieve and check visibility for.
 * @throws Throws if the visibility does not match the expected visibility state.
 */
function assertIsVisible(expected, { element, id, l10nId }) {
  if (element) {
    return is(
      isVisible(element),
      expected,
  ...
  if (l10nId) {
    return ok(
      maybeGetByL10nId(l10nId) !== null,
      expected,
      `Expected element with data-l10n-id '${l10nId}' to be ${
        expected ? "visible" : "invisible"
      }`
    );
  }

  return ok(false, "assertIsVisible() was called with no specified element.");
}
...
async function openContextMenu(
  ...
  if (openAtEnglishHyperlink === true) {
    await runInPage(async TranslationsTest => {
      ...
      console.log("English hyperlink text:", hyperlink.textContent);
  ...
  if (openAtSpanishHyperlink === true) {
    await runInPage(async TranslationsTest => {
      ...
      console.log("Spanish hyperlink text:", hyperlink.textContent);
      await TranslationsTest.rightClickContentElement(hyperlink);
    });
    return;
  }
...
async function assertContextMenuTranslateSelectionItem(
  ...
  if (expectMenuItemVisible !== undefined) {
    assertIsVisible(expectMenuItemVisible, menuItem);
  }
...
function getById(id, doc = document) {
  const element = maybeGetById(id, /* ensureVisibility */ true, doc);
  if (!element) {
 ...
 * @param {boolean} [ensureVisibility=true] - If set to true, the function will return null when the element is not visible.
 ...
 */
function maybeGetById(id, ensureVisibility = true, doc = document) {

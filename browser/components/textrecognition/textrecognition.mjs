...
ourBrowser
  .closest(".dialogBox")
  ?.classList.add("textRecognitionDialogBox", "noShadow");
ourBrowser
  .closest(".dialogOverlay")
  ?.classList.add("textRecognitionDialogOverlay");
ourBrowser.classList.add("textRecognitionDialogFrame");

const contentEl = ourBrowser
  .closest(".browserStack")
  .querySelector('browser[type="content"]');

contentEl?.classList.add("textRecognitionContentArea");
...
window.addEventListener("unload", () => {
  contentEl?.classList.remove("textRecognitionContentArea");
});
...
class TextRecognitionModal {
   ...
   */
  constructor(resultsPromise, resizeVertically, imageInfo) {
    ...
    /** @type {HTMLElement} */
    this.loadingEL = document.querySelector(".textRecognitionTextLoading");

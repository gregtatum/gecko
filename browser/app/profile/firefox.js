...
pref("genai.context.pageText", true);
pref("genai.context.selection", true);
pref("genai.debug", false);
pref("genai.debug.prompts.0", 'Find related tabs focusing on `currentTabTitle` to determine which tabs of `openTabs` are more relevant. Consider potential larger tasks these might be part of to respond JSON with 5 keys: "debug" string internal thought process considering which tabs are more relevant to the current tab, "label" string general topic of the more related tabs, "description" string 2nd-person sentence describing potential larger tasks of these more relevant tabs, "tabs" array of strings for the tab ids (domain with index, e.g., "mozilla.org2" without the title) of the 5 most related tabs from `openTabs`, "queries" array of string 5 followup context-less (i.e., search engine will not know what is "this") search queries to continue exploring the topic. { %currentTabTitle%, %openTabs% }');

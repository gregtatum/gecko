...
const PREFERRED_WIDTH = AppConstants.PINEBUILD ? 32 : 16;
  ...
  for (let icon of iconInfos) {
    if (!icon.isRichIcon) {
      ...
      if (guessType(icon) == TYPE_SVG) {
        ...
        // When the pinebuild is enabled, our tab icons are sized to 32px. At
        ...
        !(AppConstants.PINEBUILD && preferredWidth >= 64) &&

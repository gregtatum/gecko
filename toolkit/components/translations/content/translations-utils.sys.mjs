...
/**
 * Keep 1 language engine around in the cache in case page navigation happens and we
 * can re-use the previous engine. The engines are very heavy-weight, so we only
 * want to keep one around at a time.
 */

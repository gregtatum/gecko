...
class QueuedTranslator {
   ...
   * @param {Document} document
   * @param {() => void} actorRequestNewPortFn
   */
  constructor(port, actorRequestNewPortFn) {
    this.#actorRequestNewPortFn = actorRequestNewPortFn;
  ...
  showPage() {
    ...
    if (this.#port) {
      throw new Error(
        "Attempting to how the page when there is already port available"
      );
    }
    if (this.#queue.size) {
  ...
  async translate(node, sourceText, isHTML) {
    ...
    // At this point we don't know if the page is still shown, of if the attempt

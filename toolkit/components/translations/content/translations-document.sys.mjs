...
export class TranslationsDocument {
  ...
  subdivideNodeForTranslations(node) {
    ...
    switch (this.determineTranslationStatusForUnprocessedNodes(node)) {
      ...
      case NodeStatus.SUBDIVIDE_FURTHER:
        // This node can be translated, but it needs to be subdivided into smaller pieces.
        // Create a TreeWalker to walk the subtree, and find the subtrees/nodes that
        // contain enough inline elements to send to be translated.
        {
  ...
  reportWordsInViewport() {
    ...
    // TODO - Add telemetry.
    // TODO - This whitespace regex will not work in CJK-like languages. This requires a
    // segmenter for a proper implementation.
  ...
  /**
   * This is called every `DOM_UPDATE_INTERVAL` ms with translations for nodes.
   *

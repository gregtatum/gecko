 ...
 * file, You can obtain one at http://mozilla.org/MPL/2.0/. */
 ...
 //try {
 // throw new Error("This is my worker error.");
 //} catch(e) {
 // console.error(e);
 //}
 console.log("\nv\nv\nv\nv\nHELLLOOOOOOOOO\n^\n^\n^\n^\n");
...
//import { FastText } from "chrome://global/content/translations/fasttext.js";
...
async function initialize({ data }) {
  ...
  try {
    ...
    if (!modelBuffer) {
      throw new Error('Worker initialization missing "model"');
    }
    if (!wasmBuffer) {
      throw new Error('Worker initialization missing "fastTextWasmArrayBuffer"');
    }
    if (isLoggingEnabled) {
    ...
    log("Creating initial module");
    let promise = new Promise((resolve, reject) => {

      log("Loading fast text");
      const initialModule = {
        onAbort() {
          reject(new Error("Error loading the fastText Wasm Module"));
        },
        onRuntimeInitialized() {
            log("onRuntimeInitialized 1");
            log(initialModule.name);
            const ft = new FastText(initialModule);
            log("onRuntimeInitialized 2");
            //log("Ft: ", ft);
            const model = ft.loadModelBinary(modelBuffer);
            log("onRuntimeInitialized 3");
            //log("Model: ", model);
            resolve(model);
            log("onRuntimeInitialized 4");
        },
    ...
    log(model);
    log("Creating languageIdWorker");
    new LanguageIdWorker(
      model,
    );
    ...
    switch (data.type) {
      case "language-id-request": {
        ...
        try {
          ...
          log(this);
          log(this.model);
          printVector(this.model.predict(text, 5, 0.0));

          // text = "Bonjour à tous. Ceci est du français";
          // log(text);
          // printVector(this.model.predict(text, 5, 0.0));

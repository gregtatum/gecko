...
export class TranslationsParent extends JSWindowActorParent {
   ...
   * @see {nsIObserver}
   */
  static async #observeStatic(subject, topic, data) {
      ...
      case TOPIC_TRANSLATIONS_PREF_CHANGED: {
        switch (data) {
          case USE_LEXICAL_SHORTLIST_PREF: {
            ...
            // at all, much less during a translation. But if it does happen, we need to wait
            // for the engine with the old configuration to be destroyed, so that it can be
            // rebuilt with the proper files.
            await lazy.EngineProcess.destroyTranslationsEngine().catch(error =>

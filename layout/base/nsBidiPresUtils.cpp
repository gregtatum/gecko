...
struct MOZ_STACK_CLASS BidiParagraphData {
  ...
  void GetLogicalRun(int32_t aLogicalStart, int32_t* aLogicalLimit,
    ...
    *aLogicalLimit = logicalRun.limit;
    *aLevel = logicalRun.embeddingLevel;
    if (mIsVisual) {
...
nsresult nsBidiPresUtils::ProcessText(const char16_t* aText, int32_t aLength,
  ...
  if (aBidiEngine->SetParagraph(Span(text, aLength), aBaseLevel.Direction())
          .isErr()) {
    return NS_ERROR_FAILURE;
  }
  ...
  for (i = 0; i < runCount; i++) {
    ...
    auto logicalRun = aBidiEngine->GetLogicalRun(start);
    limit = logicalRun.limit;
    level = logicalRun.embeddingLevel;

...
class DisplayNames final {
 public:
  ...
  enum class Type {
    ...
    /**
     * The following are options from Intl.DisplayNames v2.
   ...
   * values must match the default initialized values of ECMA 402.
   *
 ...
 private:
#ifdef DEBUG
  /**
   * An internal (debug-only) helper for checking that a symbol is a standalone
   * month.
   */
  ...
  // The following are the stack-allocated sizes for various strings. They
  // should be large enough to fit the common cases, and it will fall back to
  // heap allocations.
  ...
  Result<Ok, DisplayNamesError> GetMonth(B& aBuffer, Span<const char> aMonth,
                                         Fallback aFallback) {
    // 13 is a valid month, as this is the "undecimber" month in lunar
    // calendars. https://en.wikipedia.org/wiki/Undecimber
    Maybe<uint8_t> month = ValidateDisplayNamesNumber(aMonth, 13);
    if (month.isNothing()) {
      return Err(DisplayNamesError::InvalidOption);
    }
    ...
    return HandleFallback(aBuffer, aFallback, [&] { return aMonth; });
  }

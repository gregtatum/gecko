...
class DisplayNames final {
 public:
  ...
  DisplayNames(ULocaleDisplayNames* aDisplayNames, Span<const char> aLocale,
    ...
    // Ensure the locale is null-terminated.
    mLocale = Buffer(Span(aLocale.data(), aLocale.size() + 1));
 ...
 public:
  ...
  Result<Ok, DisplayNamesError> GetDateTimeField(
      B& aBuffer, DateTimeField aField, Fallback aFallback = Fallback::None) {
    ...
    if (!mUDateTimePatternGen) {
      UErrorCode status = U_ZERO_ERROR;
      mUDateTimePatternGen = udatpg_open(IcuLocale(mLocale), &status);
      if (U_FAILURE(status)) {
  ...
  // Only lazily construct the date time pattern generator for GetDateTimeField.

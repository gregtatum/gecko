...
TEST(IntlDisplayNames, DateTimeField)
{
  ...
  ASSERT_TRUE(
      displayNames->GetDateTimeField(buffer, DateTimeField::Year).isOk());
  ASSERT_TRUE(buffer.verboseMatches(u"year"));
  ASSERT_TRUE(
      displayNames->GetDateTimeField(buffer, DateTimeField::Quarter).isOk());
  ASSERT_TRUE(buffer.verboseMatches(u"quarter"));
  ASSERT_TRUE(
      displayNames->GetDateTimeField(buffer, DateTimeField::Month).isOk());
  ASSERT_TRUE(buffer.verboseMatches(u"month"));
  ASSERT_TRUE(
      displayNames->GetDateTimeField(buffer, DateTimeField::WeekOfYear).isOk());
  ASSERT_TRUE(buffer.verboseMatches(u"week"));
  ASSERT_TRUE(
      displayNames->GetDateTimeField(buffer, DateTimeField::Weekday).isOk());
  ASSERT_TRUE(buffer.verboseMatches(u"day of the week"));
  ASSERT_TRUE(
      displayNames->GetDateTimeField(buffer, DateTimeField::Day).isOk());
  ASSERT_TRUE(buffer.verboseMatches(u"day"));
  ASSERT_TRUE(
      displayNames->GetDateTimeField(buffer, DateTimeField::DayPeriod).isOk());
  ASSERT_TRUE(buffer.verboseMatches(u"AM/PM"));
  ASSERT_TRUE(
      displayNames->GetDateTimeField(buffer, DateTimeField::Hour).isOk());
  ASSERT_TRUE(buffer.verboseMatches(u"hour"));
  ASSERT_TRUE(
      displayNames->GetDateTimeField(buffer, DateTimeField::Minute).isOk());
  ASSERT_TRUE(buffer.verboseMatches(u"minute"));
  ASSERT_TRUE(
      displayNames->GetDateTimeField(buffer, DateTimeField::Second).isOk());
  ASSERT_TRUE(buffer.verboseMatches(u"second"));
  ASSERT_TRUE(
      displayNames->GetDateTimeField(buffer, DateTimeField::TimeZoneName)
          .isOk());

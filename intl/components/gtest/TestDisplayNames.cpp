...
TEST(IntlDisplayNames, Month)
{
  ...
  ASSERT_TRUE(displayNames->Of(buffer, MakeStringSpan("13")).isOk());
  ASSERT_TRUE(buffer.verboseMatches(u""));
  ASSERT_TRUE(
      displayNames
          ->Of(buffer, MakeStringSpan("13"), DisplayNames::Fallback::Code)
          .isOk());
  ASSERT_TRUE(buffer.verboseMatches(u"13"));
  ...
  {
    // Above valid range.
    auto err = displayNames->Of(buffer, MakeStringSpan("14"));
    ASSERT_TRUE(err.isErr());
    ASSERT_EQ(err.unwrapErr(), DisplayNamesError::InvalidOption);
  }
}

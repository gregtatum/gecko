...
TEST(IntlBidi, SimpleLTR)
{
  ...
  ASSERT_TRUE(bidi.SetParagraph(MakeStringSpan(u"this is a paragraph"),
                                Bidi::Direction::LTR)
                  .isOk());
  ASSERT_EQ(bidi.GetParagraphLevel(), 0);
...

  {
    ...
    ASSERT_EQ(logicalRun.inspect()->Direction(), Bidi::Direction::LTR);
...
TEST(IntlBidi, SimpleRTL)
{
  ...
  ASSERT_TRUE(
      bidi.SetParagraph(MakeStringSpan(u"فايرفوكس رائع "), Bidi::Direction::LTR)
          .isOk());
  ASSERT_EQ(bidi.GetParagraphLevel(), 1);
...

  {
    ...
    ASSERT_EQ(logicalRun.inspect()->Direction(), Bidi::Direction::RTL);

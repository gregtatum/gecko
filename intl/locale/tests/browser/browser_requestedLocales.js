...
add_task(async function toolbarButtons() {
  ...
  await AddonTestUtils.promiseInstallFile(createLangpack({
    locale: "es-ES",
    files: [
      {
        alias: "branding",
  ...
  Assert.equal(sharedStringBundle.GetStringFromName("brandFullName"), "Renard de Feu");
  Assert.equal(testOnlyBundle.GetStringFromName("testOnly"), "Message de test uniquement");
});

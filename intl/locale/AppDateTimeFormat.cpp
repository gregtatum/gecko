...
nsresult AppDateTimeFormat::Format(const DateTimeFormat::ComponentsBag& aBag,
  ...
  auto genResult = DateTimePatternGenerator::TryCreate(*sLocale);
...
nsresult AppDateTimeFormat::Format(const DateTimeFormat::StyleBag& aStyle,
  ...
  if (!dateTimeFormat) {
    ...
    if (aTimeParameters) {
      ...
      timeZoneOverride = Some(Span(timeZoneID.Data(), timeZoneID.Length()));
    }

    auto result =
        DateTimeFormat::TryCreate(*sLocale, pattern, timeZoneOverride);
    NS_ENSURE_TRUE(result.isOk(), NS_ERROR_FAILURE);
